    ...
    >
      {items.map((item, index) => (
        <li
          key={item.id}
          className={cn(
            "group relative cursor-pointer overflow-hidden rounded-2xl border border-white/10 bg-card shadow-sm",
            "md:min-w-[80px]",
            "min-h-0 min-w-0"
          )}
        ...
        >
          ...
          <article className="absolute inset-0 flex flex-col justify-end gap-2 p-4">
            ...
            <h3 className="font-heading text-xl font-bold text-white opacity-0 transition-all duration-300 delay-150 ease-out group-data-[active=true]:opacity-100">
              {item.title}
            </h3>
