...
// diferido: esta sección va DESPUÉS del hero, así que su JS nunca compite
...
const INJECTED_STYLES = `
   ...
   * primer pintado. GSAP, al cargar (diferido), fija exactamente estos
   * mismos valores antes de animar por scroll — así nunca hay un salto ni
   * un flash entre el estado "sin JS" y el estado "con JS ya cargado".
   */
  .main-card {
...
export interface CasaCinematicaProps
...
/**
 * Segunda sección de la home: la casa se construye con el scroll (pin +
 * scrub). GSAP/ScrollTrigger se cargan diferidos (dynamic import) porque
 * esta sección vive DESPUÉS del hero — su JS nunca bloquea la apertura.
 */
export function CasaCinematica({
  ...
  return (
    <div ref={containerRef} className={"relative w-full h-[600vh] " + (className ?? "")} {...props}>
